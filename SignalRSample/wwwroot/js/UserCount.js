//const { signalR } = require("../lib/microsoft/signalr/dist/webworker/signalr.js");

//create connection
var connectionUserCount = new signalR.HubConnectionBuilder().configureLogging(signalR.LogLevel.Information)
    .withUrl("/hubs/userCount").build();      //to change transport type pass additional paramater to withUrl(, signalR.HttptransportType.--type--)
...
function newWindowLoadedOnClient() {
    ...
    //can use invoke in place of send only difference is send doesn't need a response but invke needs a response from server
    //connectionUserCount.invoke("NewWindowLoaded","passParamaterhere").then((value) => console.log(value));
}
